...
func GenerateName() string {
	...
	tempString := fmt.Sprintf("%s %s %s", adjectives[rand.Intn(len(adjectives))], colors[rand.Intn(len(colors))], people[rand.Intn(len(people))])
	return strings.ToTitle(tempString)
}

func GenerateEmail() string {
	rand.Seed(time.Now().UnixNano())
	return fmt.Sprintf("%s@%s", people[rand.Intn(len(people))], "gmail.com")
}
